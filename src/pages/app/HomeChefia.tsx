...
import { FileText, AlertCircle, FolderOpen, DollarSign, TrendingUp, Wallet, Activity } from 'lucide-react';
...
export function HomeChefia() {
    ...
    const getCardColors = (cardName: string) => {
        const name = cardName.toLowerCase();
        if (name.includes('ef')) return { iconColor: 'text-blue-600 dark:text-blue-400', iconBgColor: 'bg-blue-100 dark:bg-blue-900/30' };
        if (name.includes('demanda')) return { iconColor: 'text-orange-600 dark:text-orange-400', iconBgColor: 'bg-orange-100 dark:bg-orange-900/30' };
        if (name.includes('cda') && !name.includes('soma')) return { iconColor: 'text-violet-600 dark:text-violet-400', iconBgColor: 'bg-violet-100 dark:bg-violet-900/30' };
        if (name.includes('soma') && name.includes('cda')) return { iconColor: 'text-green-600 dark:text-green-400', iconBgColor: 'bg-green-100 dark:bg-green-900/30' };
        if (name.includes('pagamento')) return { iconColor: 'text-emerald-600 dark:text-emerald-400', iconBgColor: 'bg-emerald-100 dark:bg-emerald-900/30' };
        if (name.includes('repasse')) return { iconColor: 'text-cyan-600 dark:text-cyan-400', iconBgColor: 'bg-cyan-100 dark:bg-cyan-900/30' };
        return { iconColor: 'text-gray-600 dark:text-gray-400', iconBgColor: 'bg-gray-100 dark:bg-gray-900/30' };
    };
            ...
            <div className="relative h-screen overflow-hidden -mt-16">
                    ...
                    <div className="relative w-full max-w-[1400px] aspect-[16/10]">
                        ...
                        <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 z-10">
                            <div className="relative p-8 bg-white dark:bg-gray-800 rounded-full shadow-2xl border-4 border-violet-200 dark:border-violet-800">
                                <img src={logo} alt="Logo SIDA" className="w-48 h-48 md:w-64 md:h-64 object-contain" />
                ...
                <div className="xl:hidden relative w-full max-w-md mx-auto py-8 px-4 overflow-y-auto h-full">
                    <div className="fixed inset-0 flex items-center justify-center pointer-events-none overflow-hidden z-0">
                        <img src={logo} alt="Logo SIDA Background" className="w-[400px] h-auto opacity-[0.05] dark:opacity-[0.03] select-none" />
                    </div>
